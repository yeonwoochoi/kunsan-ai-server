...
exports.getBoardContentInPage = async (req, res, next) => {
    ...
    if (!keyword) {
        const query = `SELECT * FROM board ORDER BY ${sortBy} DESC LIMIT ${(currentPage-1) * itemPerPage}, ${itemPerPage}`;
        ...
        await setSearchConditions(searchBy, keyword).then(
            (conditionQuery) => {
                if (conditionQuery.length === 0) {
                    ...
                    let query = `SELECT * FROM board WHERE ${conditionQuery} ORDER BY ${sortBy} DESC LIMIT ${(currentPage-1) * itemPerPage}, ${currentPage * itemPerPage}`
                    connection.query(query, async function (error, results, fields) {
                        if (error) {
